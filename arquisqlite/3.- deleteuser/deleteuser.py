...
def recibir(sock, addr):
    ...
    while True:
        ...
        consulta0 = cursorObj.execute('SELECT tipodeusuario FROM usuario where email="{data[0]}"')
        ...
        if tipousuario == "True":
            ...
            consulta1= cursorObj.execute('SELECT idusuario FROM usuario where email="{data[1]}"')
            ...
            if idusuario1=="":
                ...
                consulta2 = cursorObj.execute('DELETE FROM usuario WHERE email="{data[1]}"')
                respuesta = modificar(consulta2)
                # obtener ids de las mascotas del dueño
                consulta3 = cursorObj.execute('SELECT mascota.idmascota FROM mascota, usuariomascota WHERE usuariomascota.idusuario="{idusuario1}" AND usuariomascota.idmascota=mascota.idmascota')
                ...
                if len(listaids) > 0:
                    for id in listaids:
                        con = cursorObj.execute('DELETE FROM mascota WHERE idmascota="{id}"')
                        modificar(con)
                        con2 = cursorObj.execute('DELETE FROM usuariomascota WHERE idmascota={id}')
                        modificar(con2)
